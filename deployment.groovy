...
def harvest_log() {
    stage('harvest_log') {
        ...
        sh '''nsre -m setup -c /tmp/nsre-$$.yaml -url https://10.100.9.223 -f ${BUILD_TAG}.log,${JENKINS_HOME}/jobs/${JOB_NAME}/builds/${BUILD_NUMBER}/log -jwtkey ${NSRE_JWT_API_KEY} -appname ${BUILD_TAG}
...
def run_build_script(arg1=[:]) {
    def default_arg = ['docker_net_opt': '--net=container:xvt', 'docker_volume_opt': '--volumes-from xvt_jenkins', 'docker_image': 'xvtsolutions/python3-aws-ansible:2.8.5', 'extra_build_scripts': [], 'run_as_user': [:] ]
